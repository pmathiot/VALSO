...
for AREA in GETZ AMUS; do
   i=$((i+1))
   echo "plot mean bot T (${AREA}) time series"
   python SCRIPT/plot_time_series.py -noshow -runid $RUNIDS -f ${AREA}*thetao*${FREQ}*T.nc   -var '(mean_sosbt_tmask|mean_sbt_tmask)' -title "Mean bot. temp. ${AREA} (C)" -dir ${WRKPATH} -o ${KEY}_fig0${i} -obs OBS/${AREA}_botT_mean_obs.txt
   if [[ $? -ne 0 ]]; then exit 42; fi
done
...
for AREA in GETZ AMUS; do
   ...
   python SCRIPT/plot_time_series.py -noshow -runid $RUNIDS -f ${AREA}*so*${FREQ}*T.nc   -var '(mean_sosbs_tmask|mean_sbs_tmask)' -title "Mean bot. sal. ${AREA} (g/kg)" -dir ${WRKPATH} -o ${KEY}_fig0${i}  -obs OBS/${AREA}_botS_mean_obs.txt
   if [[ $? -ne 0 ]]; then exit 42; fi
done

i=$((i+1))
echo 'plot SIE time series'
python SCRIPT/plot_time_series.py -noshow -runid $RUNIDS -varf 'AMUXL*sie*m02*.nc' 'AMUXL*sie*m09*.nc' -var SExnsidc SExnsidc -sf 0.001 0.001 -title "SIE AMUXL m02 [1e6 km2]" "SIE AMUXL m09 [1e6 km2]" -dir ${WRKPATH} -o ${KEY}_fig09 #-obs OBS/AMUXL_sie02_obs.txt
if [[ $? -ne 0 ]]; then exit 42; fi
...
convert FIGURES/box_VALAMU.png -trim -bordercolor White -border 40 tmp06.png
